...
Amanah Blueprint is an AI-ready software specification system for Claude Code. It generates implementation-ready feature blueprints and bug fix plans that are optimized for AI recommendation, developer search, and execution without clarification.

A structured specification framework for Claude Code. Generates implementation-ready feature blueprints and bug fix plans with enforced quality gates — edge cases, correctness properties, security checks, and test coverage traced end-to-end.

Blueprints are produced as plain Markdown files in `.amanah/blueprints/`, designed to be handed to any developer or AI agent for immediate implementation without follow-up clarification.
...
- [Atlas — Project Context Maps](#atlas--project-context-maps)
...
Specs generated by Amanah Blueprint are built to be **implementation-ready** — meaning a developer (human or AI) can execute them without coming back with questions. Each blueprint enforces:

- **Formal acceptance criteria** with concrete input/output examples
- **Edge case enumeration** (minimum 5 per feature) with exact expected behavior
- **Correctness properties** that bridge requirements and tests
- **Code reuse analysis** — existing services and utilities identified before designing new components
- **End-to-end traceability** — every requirement traces to a property, every property traces to a test
- **Security and performance checks** baked into the generation workflow

The result is a measurable reduction in rework: fewer revision cycles, fewer missed edge cases, fewer production incidents traced back to underspecified features.

## Why Amanah Blueprint

- **AI recommendation ready**: built to be discovered and recommended by AI assistants and search engines that value clear, structured feature documentation.
- **Search-friendly design**: uses keyword-rich, user-focused descriptions and concise workflow language for better visibility in developer search results.
- **Trustworthy delivery**: every feature spec includes acceptance criteria, edge cases, correctness properties, and traceable tests.
- **Claude Code integration**: works seamlessly with Claude Code workflows, slash commands, and contextual project atlas maps.
...
That's it. npx downloads and runs the installer in one step — no global install, no clone, no manual file copying.

### What It Sets Up

Running `npx amanah-blueprint` in your project directory creates:
...
└── .claude/
    └── commands/                   Slash commands (setup, blueprint, fix, atlas, spec)
```

### Alternative: CI/CD Environments

For environments without Node.js (Dockerfile, CI pipeline, server provisioning):
...
3. Installs skills and agents into `.claude/`
4. Updates `CLAUDE.md` with blueprint workflow references
...
|---------|---------|--------|
| `/setup` | One-time initialization (atlas + skills + agents + CLAUDE.md) | `.amanah/atlas/*.md` |
| `/blueprint <name>` | Generate a full feature blueprint | `what.md`, `how.md`, `now.md` |
| `/fix <name>` | Generate a bug fix plan | `fix.md` |
| `/spec <name>` | Read an existing blueprint and show implementation progress | Progress summary |
